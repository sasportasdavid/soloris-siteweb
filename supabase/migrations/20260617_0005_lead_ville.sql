-- ============================================================================
-- Migration 5 — Ville du bien (Soloris)
-- Additive. Colonne `ville` (déduite du code postal côté client via geo.api.gouv.fr)
-- + upsert_lead met à jour / renvoie `ville` (carte Telegram, back-office).
-- Rollback : 20260617_0005_lead_ville.down.sql
-- ============================================================================

alter table public.leads
  add column if not exists ville text; -- commune déduite du code postal (secteur)

-- ── upsert_lead : prend en charge `ville` (insert / update / retour) ──
...
begin
  ...
  if v_row.id is null then
    insert into leads (
      ...
      secteur, ville, estimation, nom, telephone, email, message, annexe, annexe_type,
      ...
      nullif(payload->>'surface','')::int, nullif(payload->>'secteur',''), nullif(payload->>'ville',''),
    ...
    update leads set
      ...
      ville = coalesce(nullif(payload->>'ville',''), ville),
  ...
  return jsonb_build_object(
    ...
    'lead', jsonb_build_object(
      ...
      'surface', v_row.surface, 'estimation', v_row.estimation, 'secteur', v_row.secteur, 'ville', v_row.ville,
